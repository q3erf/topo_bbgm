...
def train_eval_model(model, criterion, optimizer, dataloader, num_epochs, resume=False, start_epoch=0):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for inputs in dataloader["train"]:
            ...
            # topo
            loc_feat_list = [_.cuda() for _ in inputs["loc_topo_feats"]]
            glob_feat_list = [_.cuda() for _ in inputs["glob_topo_feats"]]
            graph_feat_list = [_.cuda() for _ in inputs["graph_feats"]]
            ...
            with torch.set_grad_enabled(True):
                # forward
                s_pred_list = model(data_list, points_gt_list, edges_list, n_points_gt_list,
                                    perm_mat_list, loc_feat_list, glob_feat_list, graph_feat_list)
