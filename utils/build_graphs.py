...
def build_graphs(P_np: np.ndarray, n: int, n_pad: int = None, edge_pad: int = None):
    ...
    A = delaunay_triangulate(P_np[0:n, :]) # 邻接矩阵
    edge_num = int(np.sum(A, axis=(0, 1))) # 边数
    ...
    for i in range(n):
        for j in range(n):
            if A[i, j] == 1:
                edge_list[0].append(i) # 头
                edge_list[1].append(j) # 尾
                features.append(locations_to_features_diffs(*P_np[i], *P_np[j])) # 边特征为边的向量放缩一定比例
